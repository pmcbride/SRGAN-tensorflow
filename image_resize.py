...
def downscale(name):
    ...
    with Image.open(name) as im:
        ...
        #save_name_HR = os.path.join(output_dir_HR, "2cstbonniekleiodannyhq_" + name.split('/')[-1].split('.')[0])
        save_name_HR = os.path.join(output_dir_HR, dirname + "_" + name.split('\\')[-1].split('.')[0])
        #save_name_LR = os.path.join(output_dir_LR, "2cstbonniekleiodannyhq_" + name.split('/')[-1].split('.')[0])
...
# dirs = ["mfhgbonniebillhq", "mgbfbonnieramonhq", "mshfbonniejohnnyhq", "nafbonniechadhq", "nrgbonniebrucehq"]
dirs = ["bonnie_test_small"]
for dirname in dirs:
    input_dir = "./data/bonnie/" + dirname
    output_dir_HR = './data/bonnie/train_2X_HR'
    output_dir_LR = './data/bonnie/train_2X_LR'
    scale = 4
